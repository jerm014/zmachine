...
class CursesAdapter(ScreenAdapter):
    ...
    def __init__(self):
        ...
        try:
            with open('input.txt', 'r') as f:
                lines = f.readlines()
                for line in lines:
                    # Strip any existing line endings and add CR
                    line = line.rstrip('\r\n') + chr(13)
                    self.input_stream.append(line)
    ...
    def write_to_screen(self, text: str):
        ...
        if text.startswith("Do you wish"):  # this is the quit confirmation
            self.simulate_input("Y" + chr(13))
            return
        if not text.startswith("Score:") and not text.startswith(">") \
                and not self.move \
                and not text.startswith("Your score would") \
                and not text.startswith("This score gives"):
            with open('output.txt', 'a') as f:
                f.write('\n' + text.strip())
        if text == ">":
            # the program is asking for input. give the next command in queue.
            self.simulate_input(self.input_stream[self.input_position])
            self.input_position += 1

        self.main_screen.noutrefresh()

    def simulate_input(self, text):
        """ simulate typing by placing characters into the curses buffer. """
        # Add each character to the input buffer in reverse order
    ...
    def set_color(self, window: Window,
    ...
    def read_keyboard_input(
        ...
        while buffer_pos < len(text_buffer):
            ...
            if c == 27:
                # Escape sequence.
                # For special characters (arrows, function keys), the remaining
                # characters will be in the curses input stream.
                escape_sequence = []
                ...
                if tuple(escape_sequence) in self.ZSCII_INPUT_HOTKEYS.keys():
                    ...
                    continue
                # If the hotkey is recognized as an interrupt character, write
                # it to the buffer and stop here.
                # If the input buffer length is 1 (read_char), write the
            ...
            if c == -1:
                ...
                if buffer_pos == 1 and text_buffer[0] == ord("y"):
                    break
                if buffer_pos == 1 and text_buffer[0] == ord("q"):
                    break 
                with open('output.txt', 'a') as file:
                    file.write('>')
                    file.write(''.join(chr(num) for num in text_buffer))
